...
# MAGIC %md
# MAGIC # Visualizing Customer addresses on a flood plane
# MAGIC 
# MAGIC King County (WA) publishes flood plain data as well as tax parcel data. We can use the addresses in the tax parcel data and use the geocoder to calculate coordinates. Using this coordinates and the flood plain data we can enrich out dataset with a flag indicating whether the house is in a flood zone or not.
# MAGIC 
# MAGIC The following data has been sourced from King County's Open data portal. [_Link_](https://data.kingcounty.gov/)
# MAGIC 1. [Address Data](https://mmlspark.blob.core.windows.net/publicwasb/maps/KingCountyAddress.csv)
# MAGIC 1. [Flood plains](https://mmlspark.blob.core.windows.net/publicwasb/maps/KingCountyFloodPlains.geojson)
# MAGIC 
# MAGIC For this demonstration, please follow the instructions on setting up your azure maps account from the overview notebook.

# COMMAND ----------

# MAGIC %md
# MAGIC ## Prerequisites
# MAGIC 1. Upload the flood plains data as map data to your creator resource
...
# MAGIC %md
# MAGIC Now that we have the flood plains data setup in our maps account, we can use the `CheckPointInPolygon` function to check if a location `(lat,lon)` coordinate is in a flood zone.
# MAGIC 
# MAGIC ### Load address data:
...
# MAGIC %md
# MAGIC ### Wire-up the Address Geocoder
# MAGIC 
# MAGIC We will use the address geocoder to enrich the dataset with location coordinates of the addresses.
...
# MAGIC %md
# MAGIC Now that we have geocoded the addresses, we can now use the `CheckPointInPolygon` function to check if a property is in a flood zone or not.
# MAGIC 
# MAGIC ### Setup Check Point In Polygon 
...
# MAGIC %md
# MAGIC ### Cleanup Uploaded User Data (Optional)
# MAGIC You can (optionally) delete the uploaded geojson polygon.
