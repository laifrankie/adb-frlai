...
# MAGIC %md
# MAGIC # Twitter Sentiment Classification using Vowpal Wabbit in SynapseML
# MAGIC 
# MAGIC In this example, we show how to build a sentiment classification model using Vowpal Wabbit (VW) in SynapseML. The data set we use to train and evaluate the model is [Sentiment140](http://help.sentiment140.com/for-students/?source=post_page---------------------------) twitter data. First, we import a few packages that we need.
...
# MAGIC %md
# MAGIC Let's read the training data into a Spark DataFrame.
...
# MAGIC %md
# MAGIC We can take a look at the training data and check how many samples it has. We should see that there are 1.6 million samples in the training data. There are 6 fields in the training data:
# MAGIC * label: the sentiment of the tweet (0.0 = negative, 2.0 = neutral, 4.0 = positive)
# MAGIC * id: the id of the tweet
# MAGIC * date: the date of the tweet
# MAGIC * query_string: The query used to extract the data. If there is no query, then this value is NO_QUERY.
# MAGIC * user: the user that tweeted
# MAGIC * text: the text of the tweet
...
# MAGIC %md
# MAGIC ## VW SynapseML Training
# MAGIC 
# MAGIC Now we are ready to define a pipeline which consists of feature engineering steps and the VW model.
...
# MAGIC %md
# MAGIC With the prepared training data, we can fit the model pipeline as follows.
...
# MAGIC 
# MAGIC After training the model, we evaluate the performance of the model using the test set which is manually labeled.
...
# MAGIC %md
# MAGIC We only use positive and negative tweets in the test set to evaluate the model, since our model is a binary classification model trained with only positive and negative tweets.
...
# MAGIC %md
# MAGIC You should see an ROC curve like the following after the above cell is executed. 
# MAGIC 
# MAGIC <img src="https://user-images.githubusercontent.com/20047467/69376052-9b0a3380-0c77-11ea-9266-11aa44350cbe.png" width="400" height="320" />