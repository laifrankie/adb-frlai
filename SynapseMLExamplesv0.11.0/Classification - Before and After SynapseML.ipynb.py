...
# MAGIC %md
# MAGIC ## Classification - Before and After SynapseML
# MAGIC 
...
# MAGIC **`synapseml`** library.  The two methods yield the same performance,
...
# MAGIC %md
# MAGIC ### 3. Extract more features and process data
# MAGIC 
...
# MAGIC %md
# MAGIC ### 4a. Classify using pyspark
# MAGIC 
# MAGIC To choose the best LogisticRegression classifier using the `pyspark`
# MAGIC library, need to *explicitly* perform the following steps:
# MAGIC 
...
# MAGIC %md
# MAGIC ### 4b. Classify using synapseml
# MAGIC 
# MAGIC Life is a lot simpler when using `synapseml`!
# MAGIC 
...
# MAGIC 
# MAGIC 2. The **`FindBestModel`** Estimator finds the best model from a pool of
# MAGIC    trained models by finding the model which performs best on the `test`
# MAGIC    dataset given the specified metric
# MAGIC 
# MAGIC 3. The **`ComputeModelStatistics`** Transformer computes the different
