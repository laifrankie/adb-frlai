...
# MAGIC %md
# MAGIC ## Interpretability - Image Explainers
# MAGIC 
# MAGIC In this example, we use LIME and Kernel SHAP explainers to explain the ResNet50 model's multi-class output of an image.
# MAGIC 
# MAGIC First we import the packages and define some UDFs and a plotting function we will need later.
...
# MAGIC %md
# MAGIC Create a dataframe for a testing image, and use the ResNet50 ONNX model to infer the image.
# MAGIC 
# MAGIC The result shows 39.6% probability of "violin" (889), and 38.4% probability of "upright piano" (881).
...
# MAGIC %md
# MAGIC First we use the LIME image explainer to explain the model's top 2 classes' probabilities.
...
# MAGIC %md
# MAGIC We plot the LIME weights for "violin" output and "upright piano" output.
# MAGIC 
# MAGIC Green areas are superpixels with LIME weights above 95 percentile.
...
# MAGIC 
# MAGIC <img src="https://mmlspark.blob.core.windows.net/graphics/explainers/image-lime-20210811.png"/>

# COMMAND ----------

# MAGIC %md
# MAGIC Then we use the Kernel SHAP image explainer to explain the model's top 2 classes' probabilities.
...
# MAGIC %md
# MAGIC We plot the SHAP values for "piano" output and "cell" output.
# MAGIC 
# MAGIC Green areas are superpixels with SHAP values above 95 percentile.
# MAGIC 
# MAGIC > Notice that we drop the base value from the SHAP output before rendering the superpixels. The base value is the model output for the background (all black) image.
...
# MAGIC Your results will look like:
# MAGIC 
# MAGIC <img src="https://mmlspark.blob.core.windows.net/graphics/explainers/image-shap-20210811.png"/>