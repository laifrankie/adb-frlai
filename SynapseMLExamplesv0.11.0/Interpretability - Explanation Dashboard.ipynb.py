...
# MAGIC %md
# MAGIC ## Interpretability - Explanation Dashboard
# MAGIC 
# MAGIC In this example, similar to the "Interpretability - Tabular SHAP explainer" notebook, we use Kernel SHAP to explain a tabular classification model built from the Adults Census dataset and then visualize the explanation in the ExplanationDashboard from https://github.com/microsoft/responsible-ai-widgets.
# MAGIC 
# MAGIC First we import the packages and define some UDFs we will need later.
...
# MAGIC %md
# MAGIC Now let's read the data and train a simple binary classification model.
...
# MAGIC %md
# MAGIC After the model is trained, we randomly select some observations to be explained.
...
# MAGIC %md
# MAGIC Once we have the resulting dataframe, we extract the class 1 probability of the model output, the SHAP values for the target class, the original features and the true label. Then we convert it to a pandas dataframe for visualization.
# MAGIC For each observation, the first element in the SHAP values vector is the base value (the mean output of the background dataset), and each of the following element is the SHAP values for each feature.
...
# MAGIC %md
# MAGIC We can visualize the explanation in the [interpret-community format](https://github.com/interpretml/interpret-community) in the ExplanationDashboard from https://github.com/microsoft/responsible-ai-widgets/
...
# MAGIC 
# MAGIC <img src="https://mmlspark.blob.core.windows.net/graphics/rai-dashboard.png" style="float: right;"/>