...
# MAGIC %md
# MAGIC ## Interpretability - Text Explainers
# MAGIC 
# MAGIC In this example, we use LIME and Kernel SHAP explainers to explain a text classification model.
# MAGIC 
...
# MAGIC %md
# MAGIC Load training data, and convert rating to binary label.
...
# MAGIC %md
# MAGIC We train a text classification model, and randomly sample 10 rows to explain.
...
# MAGIC %md
# MAGIC First we use the LIME text explainer to explain the model's predicted probability for a given observation.
...
# MAGIC %md
# MAGIC Then we use the Kernel SHAP text explainer to explain the model's predicted probability for a given observation.
# MAGIC 
# MAGIC > Notice that we drop the base value from the SHAP output before displaying the SHAP values. The base value is the model output for an empty string.
